...
import { deleteReview, getReviewById, getReviewByUserId, updateReview } from '../../services/ReviewService';
    ...
    return (
        <section className="py-14">
            <div className="container mx-auto lg:px-0">
                <h3 className="h3 text-[45px] text-left py-12">Review History</h3>
                {/* Reviews Section */}
                            ...
                            <div className="bg-gray-100 bg-opacity-90 rounded-lg shadow-lg p-6">
                                <div className="flex justify-between items-center">
                                    <div className="flex items-center">
                                        <div className="w-16 h-16 rounded-full bg-gray-300">{item.user.imageUrl ? (
                                            <img
                                        ...
                                        <div className="ml-4">
                                            ...
                                            <div className="flex space-x-1">
                                                <span className="text-yellow-500">{renderStars(item.reviewRate)}</span> {/* Rating */}
                                            </div>
                                        </div>
                                    </div>
            ...
            <Modal open={isModalUpdateOpen} onClose={() => setModalUpdateOpen(false)}>
                ...
                >
                    <div className="absolute top-0 right-0 p-2 cursor-pointer" onClick={() => setModalUpdateOpen(false)}>
                        <FaTimes size={24} color="gray" />
                    </div>
                    <h2 className="text-center mb-4 h3 text-[20px] text-center">UPDATE REVIEW</h2>
                    <HoverRating
                        value={reviewData.reviewRate}
                        onChange={(event, newValue) => setReviewData({ ...reviewData, reviewRate: newValue })}
                    />
                    <TextField
                        label="Comment"
                        multiline
